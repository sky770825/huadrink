-- HUADRINK 完整資料庫重建腳本
-- 在 Supabase SQL Editor 執行: https://supabase.com/dashboard/project/kwxlxjfcdghpguypadvi/sql/new
...
CREATE TRIGGER update_registrations_updated_at
  BEFORE UPDATE ON huadrink.registrations
  FOR EACH ROW EXECUTE FUNCTION huadrink.update_updated_at_column();

CREATE TRIGGER update_system_settings_updated_at
  BEFORE UPDATE ON huadrink.system_settings
  FOR EACH ROW EXECUTE FUNCTION huadrink.update_updated_at_column();
...
(104,'謝欣蓉/小(104,'謝欣蓉/小布','毛一本唐揚茶漬-新竹'),(105,'陳俊翔AK','紫微風水命理規劃'),(106,'范藝馨','理財型房貸-台北富邦銀行'),
...
SELECT tablename FROM pg_tables WHERE schemaname = 'huadrink';
SELECT 'Indexes created:' as info;
SELECT indexname FROM pg_indexes WHERE schemaname = 'huadrink';
SELECT 'Members count:' as info, COUNT(*) FROM huadrink.internal_members;
