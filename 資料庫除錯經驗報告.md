...
## 八、2026/02 專案優化與程式碼修正

> 依據專案檢視報告（PROJECT_REVIEW.md）執行之調整，補充至本除錯經驗報告。

### 8.1 React Query 與資料同步

| 項目 | 修正前 | 修正後 |
|------|--------|--------|
| **cacheTime 棄用** | `useSystemSettings` 使用 `cacheTime`（v5 已棄用） | 改為 `gcTime` |
| **付款頁資料不同步** | `useUpdateRegistration` 僅 invalidate `['registrations']` | 同時 invalidate `['registrations','payment-eligible']` 與 `['registration', id]` |
| **建立/刪除報名** | `useCreateRegistration`、`useDeleteRegistration` 未 invalidate payment-eligible | 新增 invalidate `['registrations','payment-eligible']`，確保付款頁即時反映 |

**影響**：後台更新付款狀態後，夥伴開啟內部付款頁會立即看到正確名單；詳情 modal 更新後不會顯示舊資料。

### 8.2 Dead Code 移除

| 檔案 | 說明 |
|------|------|
| `src/components/register/Step3Form.tsx` | 報名流程僅使用 Step1、Step2，Step3Form 未被引用 |
| `src/lib/supabaseFetch.ts` | `createFetchWithTimeout` 未被任何檔案 import |

### 8.3 共用邏輯抽離

| 新增檔案 | 內容 | 使用處 |
|----------|------|--------|
| `src/lib/refCode.ts` | `generateRefCode(prefix)`，支援 HUADRINK（隨機 4 碼）、ADMIN（時間戳末 6 碼） | Register.tsx、useRegistrations.ts |
| `src/lib/registrations.ts` | `getDuplicateGroupIds()`、`sortByMemberId()` | Payment.tsx、RegistrationTable.tsx |

**目的**：消除 `sortByMemberId`、`generateRefCode` 在多處重複實作，便於維護。

### 8.4 付款憑證改為 Storage（2026/02 前期調整）

- 內部付款頁上傳改為 Supabase Storage（`payment-proofs` bucket），不再使用 base64 存入資料庫。
- bucket 檔案限制設為 50MB（`supabase/config.toml`、`scripts/create-payment-bucket.js`）。
- 新增 `npm run test-payment-storage`、`npm run cleanup-test-storage` 供測試與清理。

### 8.5 型別與表單

- `RegistrationFormData` 移除未使用的 `pay_proof_file` 欄位。

### 8.6 尚待優化（參考 PROJECT_REVIEW.md）

- 內部成員名單改為動態載入。
- Google Sheets ID 改為環境變數。
- `payment-proofs` bucket 增加 DELETE policy（改為未付款時刪除舊檔）。
- RegistrationTable、Dashboard 等大型元件模組化拆分。
...
- `PROJECT_REVIEW.md`：專案檢視報告（建議調整、風險、模組化）
